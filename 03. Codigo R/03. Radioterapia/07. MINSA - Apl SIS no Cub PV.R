...
for (i in 1:nrow(Aux_Cal)) {
  ...
  if(min_AC==Inf){
    ...
    Aux_Cal = Aux_Cal[, setdiff(colnames(Aux_Cal), IPRESS_ONC[Pob_IPRESS_RT>Pct_Limite, IPRESS_ABREV])] # Retiramos las IPRESS que hayan excedido su capacidad
...
Aux_TB_RE = data.table(RT_Ext_MINSA)[, .(Distritos=.N,
                                               Aj.A_Sis_Onc_RE=sum(Aj.Ap.SIS_R),
                                               Equip_RE=unique(Equipos_RE)), keyby=.(Prox, Categoria)]
Aux_TB_RE[, Apl_Posibles_RE:=Equip_RE*240*60]
Aux_TB_RE = Aux_TB_RE[, c("Prox", "Categoria", "Equip_RE", "Apl_Posibles_RE", "Distritos", "Aj.A_Sis_Onc_RE")]



Tb_01 = data.table(RT_Ext_MINSA)[is.na(Prox)==F, .(Distritos_At=.N,
                                                         Apl_Sis_Onc_RE_At=sum(Aj.Ap.SIS_R)), keyby=.(mcrrgn_)]
Tb_02 = data.table(RT_Ext_MINSA)[, .(Distritos=.N,
                                           Apl_Sis_Onc_RE=sum(Aj.Ap.SIS_R)), keyby=.(mcrrgn_)]
...
p_RE.Ext_MINSA = ggplot(RT_Ext_MINSA) + geom_sf(aes(fill=Prox)) + 
  ...
  labs(title="IPRESS MINSA CON ATENCION EN RADIOTERAPIA EXTERNA", subtitle=paste0("PERÚ ", Anho, ": Equipos activos"),
...
p_RE.Ext_MINSA_LC = ggplot(LC_MINSA) + geom_sf(aes(fill=Prox)) + 
  ...
  geom_label_repel(data=IPRESS_ONC %>% filter(Equipos_RE>0) %>% filter(Provincia%in%c("LIMA", "CALLAO")), 
                   ...
                   segment.inflect=T, segment.square=T, segment.size=0.4, max.overlaps=Inf) +
  scale_fill_manual(values=Col_R.ES, name="IPRESS") + 
  labs(title="IPRESS MINSA CON ATENCION EN RADIOTERAPIA EXTERNA", subtitle=paste0("LIMA METROPOLITANA Y CALLAO ", Anho, ": Equipos activos"),
       x="", y="", fill="IPRESS - RE") + theme_minimal()


write.xlsx(list(Aux_TB_RE=Aux_TB_RE, Tb_merge=Tb_merge), 
           paste0("09. Modelo RT\\",Carp,"Anho ",Anho,"\\01. MINSA Ajustado RE.xlsx"))


rm(Aux_Cal, Aux_col, Aux_row, Aux_TB_RE, Col_R.ES, i, IPRESS_ONC_23, IPRESS_ONC_30, IPRESS_ONC_35,
   Time_35, Time_30, Time_23, Time, Tb_merge, Tb_02, Tb_01, Relacion, Poblc, RT_Ext_MINSA, Medida_Usar,
   min_AC, LC_MINSA)
