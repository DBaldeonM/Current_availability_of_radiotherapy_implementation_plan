...
Grafico_Menor.Distancia = function(df_Min_Model){
  ...
  p_RE.Ext_MINSA = ggplot(RT_Ext_MINSA) + geom_sf(aes(fill=Prox)) + 
    ...
    labs(title="IPRESS MINSA CON ATENCION EN RADIOTERAPIA EXTERNA", 
         subtitle=paste0("PERÚ ", Anho, ": Equipos activos"),
         x="", y="", fill="IPRESS - RE") + 
  ...
  p_RE.Ext_MINSA_LC = ggplot(LC_MINSA) + geom_sf(aes(fill=Prox)) + 
    ...
    geom_label_repel(data=IPRESS_ONC_Sim %>% filter(Equipos_RE>0) %>% filter(IPRESS_ABREV%in%c("INEN", "HMA", "HNHU", "HNDAC", "HSEB")), 
                     ...
                     segment.inflect=T, segment.square=T, segment.size=0.4, max.overlaps=Inf) +
    labs(title="IPRESS MINSA CON ATENCION EN RADIOTERAPIA EXTERNA",
         subtitle=paste0("LIMA METROPOLITANA Y CALLAO ", Anho, ": Equipos activos"),
         x="", y="", fill="IPRESS - RE") + 
    scale_fill_manual(values = Col_R.ES, name = "Institution") + 
    theme_minimal() + 
    theme(axis.text.x = element_text(color = "black"),  # Eje X en negro
...
######### ANALISIS TOTAL:
Min_Model = Result_DF %>%
  slice_min(order_by = Media_Pond, n = 20) 



pdf("Resultados plot Ensayo.pdf", width=20, height=10)
...
for (i in 1:nrow(Min_Model)) {
  ...
  print(annotate_figure(graf_maps, 
                        top=text_grob(paste0("Tiempo de desplazamiento: ", convertir_a_hms(Min_Model[i,]$Media_Pond), 
                                             "\nIPRESS: ", Min_Model[i,]$Cat_IPRESS,
                                             "\nIPRESS II: ", Min_Model[i,]$Ipress_NivelII),color = "red", face = "bold", size = 14)))
