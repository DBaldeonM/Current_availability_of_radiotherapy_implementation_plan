...
L.identf_HNDAC = c("CALLAO_CALLAO_CALLAO", "CALLAO_CALLAO_BELLAVISTA", "CALLAO_CALLAO_CARMEN DE LA LEGUA REYNOSO", 
                   "CALLAO_CALLAO_LA PERLA", "CALLAO_CALLAO_LA PUNTA", "CALLAO_CALLAO_VENTANILLA", "CALLAO_CALLAO_MI PERU")

if ("HNDAC"%in%names(Medida_Usar)) {
  Relacion = rbind(Relacion, data.frame(Identf=L.identf_HNDAC, Prox="HNDAC", Medida_API=Medida_Usar[L.identf_HNDAC, "HNDAC"]))
}
Aux_Cal = Aux_Cal[setdiff(rownames(Aux_Cal), L.identf_HNDAC), 
                  setdiff(colnames(Aux_Cal), "HNDAC")]
...
####### MAPA
Col_R.ES = c("INEN"="#7CAE00", "HMA"="#EA8331", "HNCH"="#00BFC4", "HNHU"="#F8766D", "HNDAC"="#DDB509",
             "HRL.FSAI"="#339900", "HT"="#768E00", "HAP.CSR"="#08CE9A", "HRP"="#C77CFF",
             "IREN.CENTRO"="#9590FF", "HRI"="#BF825D", 
             "HRL"="#ffcc00", "IREN.NORTE"="#35A2FF","HBT"="#36ACC8", "HRA.MAMLL"="#C09B00",
             
             "IREN.SUR"="#FF62BC", "HG"="#D89000", "HRHDE"="#C77CFF", "HADC"="#36ACC8", "HRMNB"="#4d8e6f")
...
p_RE.Ext_MINSA = ggplot(RT_Ext_MINSA) + geom_sf(aes(fill=Prox)) + 
  ...
  labs(title="IPRESS MINSA - RADIOTERAPIA EXTERNA", subtitle=paste0("Equipos activos - ", Anho),
       x="", y="", fill="IPRESS - RE") + theme_minimal() + theme(legend.position='none')
...
p_RE.Ext_MINSA_LC = ggplot(LC_MINSA) + geom_sf(aes(fill=Prox)) + 
  ...
  geom_label_repel(data=IPRESS_ONC %>% filter(Equipos_RE>0) %>% filter(Provincia%in%c("LIMA", "CALLAO")), 
                   ...
                   segment.inflect=T, segment.square=T, segment.size=0.4, max.overlaps=Inf) +
  scale_fill_manual(values=Col_R.ES, name="IPRESS") + 
  labs(title="IPRESS MINSA - RADIOTERAPIA EXTERNA", subtitle=paste0("LIMA METROPOLITANA Y CALLAO - ", Anho),
       x="", y="", fill="IPRESS - RE") + theme_minimal()

p_RE.Ext_MINSA_LC_2 = ggplot(LC_MINSA) + geom_sf(aes(fill=Prox)) + 
  geom_label_repel(data=IPRESS_ONC %>% filter(Equipos_RE>0) %>% filter(Provincia%in%c("LIMA", "CALLAO")), 
                   aes(x=LONGITUD, y=LATITUD, label=IPRESS_ABREV, fill=IPRESS_ABREV),
                   size=7, min.segment.length=0, arrow = arrow(length = unit(0.005, "npc")),
                   segment.inflect=T, segment.square=T, segment.size=0.4, max.overlaps=Inf) +
  scale_fill_manual(values=Col_R.ES, name="IPRESS") +
  labs(title="IPRESS MINSA - RADIOTERAPIA EXTERNA", subtitle=paste0("LIMA METROPOLITANA Y CALLAO - ", Anho),x="", 
...
map_gg.IPRESS = ggarrange(p_RE.Ext_MINSA, p_RE.Ext_MINSA_LC, p_RE.Ext_MINSA_LC_2, nrow=1)
print(map_gg.IPRESS)
dev.off()


write.xlsx(list(Aux_TB_RE=Aux_TB_RE, Tb_merge=Tb_merge), 
           paste0("09. Modelo RT\\Implementacion por fases ", Anho, "\\01. MINSA RE ", fase, ".xlsx"))
