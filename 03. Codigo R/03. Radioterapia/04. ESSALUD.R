...
################## ESSALUD
IPRESS_ONC = IPRESS_ONC[INSTITUCION%in%c("ESSALUD"),]
##################

if(nrow(IPRESS_ONC)==0){
  p_RE.Ext_ESSALUD = NA
  p_RE.Ext_ESSALUD_LC = NA
  Aux_TB_RE = NA
  Tb_merge = NA
  ...
  for (i in 1:nrow(Aux_Cal)) {
    ...
    if(min_AC==Inf){
      ...
      Poblc = Pob_Map_Ubi[Pob_Map_Ubi$Identf==Aux_row,]$Apl_P.EsD_RE
  ...
  RT_Ext_ESSALUD = merge(Pob_Map_Ubi, Relacion, by="Identf", all.x=TRUE)
  RT_Ext_ESSALUD = merge(RT_Ext_ESSALUD, 
                         rename(select(IPRESS_ONC, c("IPRESS_ABREV", "INSTITUCION", "Categoria", "Equipos_RE")), 
                                Prox=IPRESS_ABREV), by="Prox", all.x=TRUE)
  
  
  
  Aux_TB_RE = data.table(RT_Ext_ESSALUD)[, .(Distritos=.N,
                                             Apl_Sis_Onc_RE=sum(Apl_P.EsD_RE),
  ...
  Tb_01 = data.table(RT_Ext_ESSALUD)[is.na(Prox)==F, .(Distritos_At=.N,
                                                       Apl_Sis_Onc_RE_At=sum(Apl_P.EsD_RE)), keyby=.(mcrrgn_)]
  Tb_02 = data.table(RT_Ext_ESSALUD)[, .(Distritos=.N,
                                         Apl_Sis_Onc_RE=sum(Apl_P.EsD_RE)), keyby=.(mcrrgn_)]
  ...
  p_RE.Ext_ESSALUD = ggplot(RT_Ext_ESSALUD) + geom_sf(aes(fill=Prox)) + 
    ...
    theme(axis.text.x = element_text(color = "black"),  # Eje X en negro
          ...
          plot.margin = unit(c(0.3, 0.3, 0.3, 0.3), "cm")) +  # Márgenes iguales  
    annotate("text", x = -80, y = 0,label = "B", size = 13, family = "Calibri") 
  
  LC_ESSALUD = RT_Ext_ESSALUD %>% filter(NOMBPROV%in%c("LIMA", "CALLAO"))
  p_RE.Ext_ESSALUD_LC = ggplot(LC_ESSALUD) + geom_sf(aes(fill=Prox)) + 
    geom_text_repel(data=LC_ESSALUD, aes(x=centroid_LON, y=centroid_LAT, label=NOMBDIS),
    ...
    labs(title="IPRESS ESSALUD CON ATENCION EN RADIOTERAPIA EXTERNA", subtitle=paste0("LIMA METROPOLITANA Y CALLAO ", Anho, ": Equipos activos"), x="", 
         y="", fill="IPRESS - RE") + theme_minimal()
  
}

write.xlsx(list(Aux_TB_RE=Aux_TB_RE, Tb_merge=Tb_merge), 
           paste0("09. Modelo RT\\",Carp,"Anho ",Anho,"\\02. ESSALUD RE.xlsx"))
