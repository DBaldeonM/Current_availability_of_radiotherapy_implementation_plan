...
Tb_01 = data.table(RT_Ext_MINSA)[is.na(Prox)==F, .(Distritos_At=.N,
                                                         Apl_Sis_Onc_RE_At=sum(Apl_P.SIS_RE)), keyby=.(mcrrgn_)]
Tb_02 = data.table(RT_Ext_MINSA)[, .(Distritos=.N,
                                           Apl_Sis_Onc_RE=sum(Apl_P.SIS_RE)), keyby=.(mcrrgn_)]
...
p_RE.Ext_MINSA = ggplot(RT_Ext_MINSA) + geom_sf(aes(fill=Prox)) + 
  ...
  theme(axis.text.x = element_text(color = "black"),  # Eje X en negro
        ...
        plot.margin = unit(c(-0.18, -0.18, -0.18, -0.18), "cm")) +  # Márgenes iguales  
...
write.xlsx(list(Aux_TB_RE=Aux_TB_RE, Tb_merge=Tb_merge), 
           paste0("09. Modelo RT\\",Carp,"Anho ",Anho,"\\01. MINSA RE.xlsx"))
